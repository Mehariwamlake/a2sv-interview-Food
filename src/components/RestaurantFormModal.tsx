...
export default function RestaurantFormModal({ initial, onClose, onSave }: Props) {
  ...
  function validate() {
    ...
    if (!(status === 'Open Now' || status === 'Closed')) e['restaurant_status'] = "Restaurant Status must be 'Open Now' or 'Closed'"
    setErrors(e)
    ...
    <div className="modal-overlay">
      <div className="modal" role="dialog" aria-modal="true">
        ...
        <form onSubmit={handleSubmit}>
          <label>
            Restaurant Name
            <input name="restaurant_name" value={name} onChange={e => setName(e.target.value)} placeholder="Enter restaurant name" className="restaurant-input" aria-describedby={errors['restaurant_name'] ? 'restaurant_name-error' : undefined} />
            {errors['restaurant_name'] && <div id="restaurant_name-error" className="error">{errors['restaurant_name']}</div>}
          </label>

          <label>
            Rating
            <input name="restaurant_rating" value={rating} onChange={e => setRating(e.target.value)} placeholder="Rating (1-5)" className="restaurant-input" aria-describedby={errors['restaurant_rating'] ? 'restaurant_rating-error' : undefined} />
            {errors['restaurant_rating'] && <div id="restaurant_rating-error" className="error">{errors['restaurant_rating']}</div>}
          </label>

          <label>
            Image URL
            <input name="restaurant_image" value={image} onChange={e => setImage(e.target.value)} placeholder="Enter restaurant logo URL" className="restaurant-input" aria-describedby={errors['restaurant_image'] ? 'restaurant_image-error' : undefined} />
          ...
          <div className="modal-actions">
            ...
            <button type="submit" className="restaurant-btn" disabled={saving}>{saving ? (initial ? 'Updating Food...' : 'Adding Food...') : (initial ? 'Update' : 'Add')}</button>
          </div>
        </form>
      </div>
    </div>
